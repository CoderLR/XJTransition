...
extension XJTransitionManager {
    ...
    func pageTransitionAnimation(type: XJTransitionAnimationType) {
        ...
        switch type {
        case .pageFromLeft:
            point = CGPoint(x: 0, y: 0.5)
        case .pageFromRight:
            point = CGPoint(x: 1, y: 0.5)
        case .pageFromBottom:
        ...
        UIView.animate(withDuration: self.animationTime) {
            ...
            case .pageFromRight:
                tempView.layer.transform = CATransform3DMakeRotation((.pi * 0.5), 0, 1, 0)
            case .pageFromBottom:
                tempView.layer.transform = CATransform3DMakeRotation(-(.pi * 0.5), 1, 0, 0)
            case .pageFromTop:
                tempView.layer.transform = CATransform3DMakeRotation((.pi * 0.5), 1, 0, 0)
            default: break
            }
...
//        guard let tempFromView = fromVc?.view?.snapshotView(afterScreenUpdates: false) else { return }
        ...
        print("fragmentViews = \(fragmentViews.count)")
        toView.isHidden = false
        fromView.isHidden = true
...
// MARK: - present动画
extension XJTransitionManager {
    func presentTransitionAnimation(type: XJTransitionAnimationType) {
        guard let containerView = transitionContext?.containerView else { return }
        guard let fromView = fromVc?.view else { return }
        guard let toView = toVc?.view else { return }
        guard let tempView = toVc?.view?.snapshotView(afterScreenUpdates: true) else { return }
        ...
        case .presentFromBottom:
            rect = CGRect(x: 0, y: screenH, width: screenW, height: screenH)
        case .presentFromTop:
            rect = CGRect(x: 0, y: -screenH, width: screenW, height: screenH)
        case .presentFromRight:
            rect = CGRect(x: screenW, y: 0, width: screenW, height: screenH)
        case .presentFromLeft:
            rect = CGRect(x: -screenW, y: 0, width: screenW, height: screenH)
        default: break
        }
        
//        tempView.frame = CGRect(x: 0 , y: screenH, width: screenW, height: screenH)
        tempView.frame = rect
        ...
        UIView.animate(withDuration: self.animationTime) {
//            tempView.layer.transform = CATransform3DMakeTranslation(0, -screenH, 0)
            switch type {
            case .presentFromBottom:
                tempView.layer.transform = CATransform3DMakeTranslation(0, -screenH, 0)
            case .presentFromTop:
                tempView.layer.transform = CATransform3DMakeTranslation(0, screenH, 0)
            case .presentFromRight:
                tempView.layer.transform = CATransform3DMakeTranslation(-screenW, 0, 0)
            case .presentFromLeft:
                tempView.layer.transform = CATransform3DMakeTranslation(screenW, 0, 0)
            default: break
            }
    ...
    func presentTransitionBackAnimation(type: XJTransitionAnimationType) {
        ...
        tempView.frame = CGRect(x: 0 , y: 0, width: screenW, height: screenH)
